...
public class EndToEndTest1 
{
	...
	public void test()
	{
		...
		Assert.assertEquals(200, response.statusCode());
		jpath = response.jsonPath();
		Assert.assertTrue(jpath.get("name").equals("Sai"));
		
		updateEmployeeName(employeeId, "Venkat");
		System.out.println(response.getBody().asString());
		Assert.assertEquals(200, response.statusCode());
		response = GetSingleEmployee(employeeId);
		jpath = response.jsonPath();
		String name = jpath.get("name");
		Assert.assertTrue(name.equals("Venkat"));
		...
		jpath = response.jsonPath();
		names = jpath.get("name");
		Assert.assertFalse(names.contains("Venkat"));
	}
